...
class KDTree
{
  private:
    ...
    struct KDTreeNode
    {
      ...
      KDTreeNode(const KDTreeNode &other) : point(other.point){
        if(other != NULL) {
    ...
    //recursive function that builds tree and returns root node
    KDTreeNode * make_tree(vector<Point<Dim>> &pts, int start, int end, int d, KDTreeNode *  root);

    //Partitions around point in middle position between start and end
    int partition(vector<Point<Dim>> &pts, int start, int end, int d, int k);

    //returns index of k'th point withing bounds [start,end) across dimension d in pts. Partitions around k'th point in the process
    Point<Dim> select(vector<Point<Dim>> &pts, int start, int end, int d, int k);
  ...
  public:
    ...
    // KDTreeNode create(KDTreeNode root, vector<Point<Dim>> &newPoints, int start, int end, int d);
    // KDTreeNode select(vector<Point<Dim>>& pts, int start, int end, int d);
    // KDTreeNode partition(vector<Point<Dim>>& pts, int start, int end, int k, int d);
    /**
     ...
     */
    Point<Dim> findNearestNeighbor(KDTreeNode* root, Point<Dim> query, int d) const;
